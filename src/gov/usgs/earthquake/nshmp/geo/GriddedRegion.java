...
public class GriddedRegion extends Region implements Iterable<Location> {
  ...
  public int move(int index, Direction dir) {
    ...
    Location end = Location.create(
        start.latitude + latSpacing * dir.signLatMove(),
        start.longitude + lonSpacing * dir.signLonMove());
  ...
  private List<Integer> indexLookupFast(Rectangle2D rect) {
    ...
    while (rowStartIndex <= lastRowStartIndex) {
      ...
      Location nextLoc = Location.create(currLoc.latitude + latSpacing, currLoc.longitude);
  ...
  public int indexForLocation(Location loc) {
    int lonIndex = nodeIndex(lonEdges, loc.longitude);
    if (lonIndex == -1) {
      return -1;
    }
    int latIndex = nodeIndex(latEdges, loc.latitude);
    if (latIndex == -1) {
      return -1;
    }
  ...
  private void setAnchor(Location anchor) {
    ...
    if (anchor == null) {
      anchor = Location.create(bounds.min.latitude, bounds.min.longitude);
    }
    double lat = computeAnchor(bounds.min.latitude, anchor.latitude, latSpacing);
    double lon = computeAnchor(bounds.min.longitude, anchor.longitude, lonSpacing);
  ...
  private void initNodes() {
    ...
    double[] lonNodes = initNodeCenters(anchor.longitude, bounds.max.longitude, lonSpacing);
    double[] latNodes = initNodeCenters(anchor.latitude, bounds.max.latitude, latSpacing);

    // node edge arrays
    lonEdges = initNodeEdges(anchor.longitude, bounds.max.longitude, lonSpacing);
    latEdges = initNodeEdges(anchor.latitude, bounds.max.latitude, latSpacing);
    ...
    nodes = LocationList.copyOf(nodeList);
    nodeCount = nodeIndex;
  }
